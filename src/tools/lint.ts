...
// interpretation, no mutation. See CLAUDE.md "library_lint".
...
async function lintImpl(input: unknown): Promise<DomainEnvelope> {
  ...
  for (const p of pages) {
    for (const r of p.related || []) {
      if (!allFilenames.has(r)) {
        issues.push({ type: 'broken_reference', page: p.filename, description: `related[] references missing page "${r}"`, severity: 'error' })
      }
    }
    if (p.confidence === 'unverified' && daysSince(p.updated) > 30) {
      issues.push({ type: 'unverified_stale', page: p.filename, description: `unverified and not updated in ${Math.floor(daysSince(p.updated))} days`, severity: 'warning' })
    }
    const qd = qdrantUpdated.get(p.filename)
    if (qd !== undefined && qd !== p.updated) {
      issues.push({ type: 'stale_embedding', page: p.filename, description: `manifest updated ${p.updated} != Qdrant updated ${qd}`, severity: 'warning' })
    }
    if (!pageFiles.has(p.filename)) {
      issues.push({ type: 'index_entry_missing_page', page: p.filename, description: 'manifest entry has no corresponding page blob', severity: 'error' })
    }
  }
  ...
  for (const p of pages) {
    ...
    if (!p.sources || p.sources.length === 0) {
      issues.push({ type: 'missing_source_metadata', page: p.filename, description: 'active page has empty or absent sources[]', severity: 'warning' })
    }
    if ((p.type === 'concept' || p.type === 'synthesis') && !inbound.has(p.filename)) {
      issues.push({ type: 'orphan_page', page: p.filename, description: 'no inbound related[] links from other active pages', severity: 'info' })
    }
  }
  ...
  for (const p of pages) {
    ...
    if (p.status === 'active' && !/\[source:[^\]]*\]/.test(body)) {
      issues.push({ type: 'inline_citation_missing', page: p.filename, description: 'page body contains no [source: ...] citation', severity: 'info' })
    }
    if (p.type === 'contradiction' && !/resolution:/i.test(body)) {
      issues.push({ type: 'open_contradiction', page: p.filename, description: 'contradiction page body has no "resolution:"', severity: 'error' })
    }
  }
  ...
  for (const s of rawManifest.sources) {
    ...
    if (s.embedding_status === 'failed' || s.chunks_indexed === 0) {
      issues.push({ type: 'source_not_indexed', source_id: s.source_id, description: `source not indexed (embedding_status ${s.embedding_status}, chunks_indexed ${s.chunks_indexed})`, severity: 'error' })
    }
  }
...
export const lintTool: ToolDefinition = {
  ...
  description:
    ...
    'sources, and manifest/blob drift. Returns a list of mechanical issues.',
  inputSchema,
